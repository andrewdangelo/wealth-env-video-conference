...
import * as e2e from "./security/e2e";
...
export default class SFU {
  ...
  async join({ roomId, videoPerms, audioPerms }) {
    this._roomId = roomId;
    ...
    //Opens a new websocket on the same link / port as the URL
    const protooTransport = new protooClient.WebSocketTransport(
      this._protooUrl
    );

    //generates a new protoo peer on the websocket just created
    ...
    this._protoo.on("open", () => {
      this._joinRoom({ audioPerms, videoPerms });
    });
  ...
  async muteMic() {
    ...
    try {
      ...
      console.error( `Error pausing server-side mic Producer: ${error}`)
  ...
  async unmuteMic() {
    ...
    try {
      ...
      console.error( `Error resuming server-side mic Producer: ${error}`)
  ...
  async changeWebcam() {
    ...
    try {
      ...
      console.error( `Could not change webcam: ${error}`)
  ...
  async setMaxSendingSpatialLayer(spatialLayer) {
    ...
    try {
      ...
      console.error( `Error setting max sending video spatial layer: ${error}`)
  ...
  async setConsumerPreferredLayers(consumerId, spatialLayer, temporalLayer) {
    ...
    try {
      ...
      console.error( `Error setting Consumer preferred layers: ${error}`)
  ...
  async setConsumerPriority(consumerId, priority) {
    ...
    try {
      ...
      console.error( `Error setting Consumer priority: ${error}`)
  ...
  async requestConsumerKeyFrame(consumerId) {
    ...
    try {
      ...
      console.error( `Error requesting key frame for Consumer: ${error}`)
  ...
  async enableChatDataProducer() {
    ...
    try {
      ...
      console.error( `Error enabling chat DataProducer: ${error}`)
  ...
  async enableBotDataProducer() {
    ...
    try {
      ...
      console.error( `Error enabling bot DataProducer: ${error}`)
  ...
  async sendBotMessage(text) {
    ...
    if (!this._botDataProducer) {
      console.error( "No bot DataProducer")
    ...
    try {
      ...
      console.error( `bot DataProducer.send() failed: ${error}`)
  ...
  async changeDisplayName(displayName) {
    ...
    try {
      ...
      console.error( `Could not change display name: ${error}`)
  ...
  async applyNetworkThrottle({ uplink, downlink, rtt, secret }) {
    ...
    try {
      ...
      console.error( `Error applying network throttle: ${error}`)
  ...
  async _joinRoom({ audioPerms, videoPerms }) {
    ...
    try {
      ...
      if (this._produce) {
        ...
        if (
          ...
          if (videoPerms) this.enableWebcam();
      ...
      console.error( `Could not join the room: ${error}`)
